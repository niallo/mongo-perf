...
env.Append(LIBPATH=['mongo-cxx-driver'])
env.Append(CPPPATH=[os.path.join(env['MONGO_SRC_DIR'], 'src/third_party/boost/')])
env.Append(LIBPATH=[os.path.join(env['MONGO_SRC_DIR'], 'build/darwin/normal/third_party/boost/')])
env.Append(LIBPATH=[os.path.join(env['MONGO_SRC_DIR'], 'build/linux2/normal/third_party/boost/')])
